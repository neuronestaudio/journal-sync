...
function buildPayload(entry) {
  ...
  return {
    'Date':                       entry.date,
    'Intellectual':               formatHabit(entry.readingHabit, entry.readingHabitNote),
    'Hobby':                      formatHabit(entry.hobby, entry.hobbyNote),
    'Meditation':                 formatHabit(entry.meditate, entry.meditateNote),
    '3L Water':                   formatHabit(entry.threeLWater, entry.threeLWaterNote),
    'Toxic':                      formatHabit(entry.seshIntoxicant, entry.seshIntoxicantNote),
    'Gym':                        formatHabit(entry.gym, entry.gymNote),
    'Cardio':                     formatHabit(entry.runCardio, entry.runCardioNote),
    'MJ':                         formatHabit(entry.media, entry.mediaNote),
    'PER':                        formatHabit(entry.perso, entry.persoNote),
    'SCL':                        formatHabit(entry.social, entry.socialNote),
    'PHYS':                       formatHabit(entry.phys, entry.physNote),
    'FAM':                        formatHabit(entry.financial, entry.financialNote),
    '$':                          entry.money,
    'NOOTROPIC':                  formatHabit(entry.rit, entry.ritNote),
  };
}
